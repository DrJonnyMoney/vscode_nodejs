...
# install - ipykernel (needed for the Jupyter codeserver extension)
...
# ---------------------------
# Install Node.js, npm, and global Svelte development tools
# ---------------------------
# Switch to root to install system packages
USER root
# Install Node.js (includes npm) via NodeSource for Node 18 LTS
RUN curl -fsSL https://deb.nodesource.com/setup_18.x | bash - && \
    apt-get update && \
    apt-get install -y nodejs
# Install global packages for Svelte development (e.g. SvelteKit CLI and Vite)
RUN npm install -g @sveltejs/kit vite

# Switch back to NB_USER
USER $NB_UID

# Optionally copy home directory contents (if needed)
